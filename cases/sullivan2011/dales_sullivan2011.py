...
zsize = 2048 # << Specified domain height in Sullivan2011
dz = zsize / kmax
...
#### Write prescribed values of Sullivan2011 case to these arrays ####
for k in range(kmax):
    # temperature
    if(z[k] <= 974.):
        th[k] = 300.
...
vg[:]  = 0.
qt[:]  = 0.
tke[:] = 0. # Will be set to minimum value by DALES itself (when required)
